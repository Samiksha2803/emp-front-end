...
export class AppComponent implements OnInit{
  ...
  add(): void {
    ...
    this.http.post("http://localhost:3000", employee).subscribe(response => {
...
update() : void{
    ...
    this.http.put("http://localhost:3000", employee).subscribe(response => {
      console.log(response);
    }, error => {
      console.error(error);
    });
...
export class AppComponent implements OnInit {
  ...
  addEmployee() {
    this.http.post('http://localhost:3000/', this.emp).subscribe(
      {
        next: (res: any) => {
          this.getEmployees();
        },
